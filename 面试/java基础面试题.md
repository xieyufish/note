...
通过对java字节码的控制来实现类实例的创建，属性访问和方法执行。java中获取字节码有如下方式：
...
4. sleep必须要捕获异常，而wait没有；
5. wait方法通常用于线程间的交互，而sleep方法则用于暂停执行；
...
2. 禁止指令重排序；
3. synchronized通过加锁实现的原子性，当一个线程获取了锁之后，其他线程不能在获取这个锁知道线程执行完毕。
...
3. 标记-整理算法：标记过程与标记清理算法一样，但后续步骤不是直接对可回收对象进行清理，而是让所有存活对象向一段移动，然后直接清理掉边界以外的内存；
...
- ApplicationClassLoader（应用程序来加载器）：由sum.misc.Launcher$AppClassLoader实现，是getSystemClassLoader()方法的返回值，所以也叫它系统类加载器；负责加载用户类路径（ClassPath）上所指定的类库，程序中默认的类加载器；
